...
type InsertFileEntry = ImagePicker.ImagePickerAsset | DocumentPicker.DocumentPickerAsset;
...
export function useInsertVideo() {
  return useCallback(async (file: InsertFileEntry): Promise<VideoFileEntry> => {
...
export function useInsertSubtitle() {
  return useCallback(async (videoId: number, file: InsertFileEntry): Promise<SubtitleFileEntry> => {
    const id = createLocalId();
    const name = getFileName(file);
